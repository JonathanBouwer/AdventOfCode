...
for l in a:
    ...
    while i < len(l):
        ...
        i += 1
    key = f"{x}-{y}"
    tiles[key] = "b" if tiles.get(key, "w") == "w" else "w"

def get_adjacent_counts(x, y):
    b_count = 0
    k = f"{x+1}-{y}"
    if k in tiles and tiles[k] == "b":
        b_count += 1
    k = f"{x-1}-{y}"
    if k in tiles and tiles[k] == "b":
        b_count += 1
...
for day in range(100):
    ...
    for y in range(min_y, max_y):
        for x in range(min_x, max_x):
            b_count = get_adjacent_counts(x, y)
            key = f"{x}-{y}"
            if b_count == 2 or (b_count == 1 and key in tiles and tiles[key] == "b"):
                if x + 2 > max_x:
                    max_x = x + 2
                elif x - 2 < min_x:
                    min_x = x - 2
