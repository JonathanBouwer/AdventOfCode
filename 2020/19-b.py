...
parsed_rules = {}
def parse_rule(n, depth):
    if depth > 10:
        raise Exception

    if n in parsed_rules:
        return parsed_rules[n]

    rule = rules_raw[n].split(" ")
    rule_regex = ""
    for c in rule:
        if c[0] == '"':
            rule_regex += c[1]
            ...
            try:
                # This is a horrific hack, essentially just support 10 recusions
                # Real solution is to like build my own regex engine or split the 
                # input somehow but fuck that this works for the given input
                # THIS ACTUALLY SHOULDN'T WORK ITS WRONG IDK
                rule_regex += f"({parse_rule(int(c), depth + 1)})"
            except Exception:
                pass
    parsed_rules[n] = rule_regex
    
    return rule_regex

regex = f"^{parse_rule(0, 0)}$"
count = 0
for l in a:
    if re.match(regex, l):
        count += 1
