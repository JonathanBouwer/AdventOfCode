...
for l in a:
    color, rest = l.split(" bags contain ")
    if not color in contained_by:
        contained_by[color] = []
    inner_colors = rest.split(", ")
    for inner_color_text in inner_colors:
        words = inner_color_text.split(" ")
        inner_color = " ".join(words[1:-1])
        if inner_color == "other":
            continue
        contained_by[color].append((int(words[0]), inner_color))

def count_bags_inside(color):
    total = 0
    if color in contained_by:
        for bag in contained_by[color]:
            total += bag[0] + bag[0] * count_bags_inside(bag[1])
    return total
