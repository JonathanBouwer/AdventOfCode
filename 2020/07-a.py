...
for l in a:
    ...
    for inner_color_text in inner_colors:
        ...
        if not inner_color in contained_by:
            contained_by[inner_color] = []
        contained_by[inner_color].append(color)

seen = set()
active = contained_by["shiny gold"]
while len(active) > 0:
    current = active.pop()
    if current in seen:
        continue
    seen.add(current)
    if current in contained_by:
        active.extend(contained_by[current])
